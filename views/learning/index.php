...
<div class="space-y-8">
    ...
    <?php
        ...
        $isLevelLocked = (int)$course['level_number'] >= 4 && !in_array($userTier, ['pro', 'master']) && ($user['role'] ?? '') !== 'admin';
    ?>
    <div class="bg-slate-900 border <?= $isLevelLocked ? 'border-amber-500/30' : 'border-slate-800' ?> rounded-2xl p-6 shadow-xl space-y-6 relative overflow-hidden">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center border-b border-slate-800 pb-4 gap-4">
            <div>
                <div class="flex items-center gap-2 mb-1">
                    ...
                    <?php if ($isLevelLocked): ?>
                        <span class="bg-amber-500/20 text-amber-400 font-extrabold text-[10px] px-2.5 py-0.5 rounded-full border border-amber-500/30 uppercase">
                            🔒 PRO / MASTER PLAN REQUIRED
                        </span>
                    <?php endif; ?>
                </div>
            ...
            <div class="flex items-center gap-3">
                <?php if ($isLevelLocked): ?>
                    <a href="/pricing" class="bg-amber-500 hover:bg-amber-400 text-slate-950 font-black px-4 py-2 rounded-xl text-xs shadow-lg transition flex items-center gap-1.5">
                        🔒 UNLOCK WITH PRO &rarr;
                    </a>
                <?php elseif (!empty($course['certificate'])): ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="bg-slate-950 border border-slate-800 p-5 rounded-xl space-y-3 hover:border-indigo-500/40 transition flex flex-col justify-between <?= $isLevelLocked ? 'opacity-70' : '' ?>">
                <div class="space-y-2">
                ...
                <div class="pt-3 border-t border-slate-900 flex justify-between items-center">
                    ...
                    <?php if ($isLevelLocked): ?>
                        <a href="/pricing" class="bg-amber-500/20 text-amber-400 hover:bg-amber-500 hover:text-slate-950 font-bold text-xs px-3 py-1.5 rounded-lg transition border border-amber-500/30">
                            🔒 UNLOCK &rarr;
                        </a>
                    <?php else: ?>
