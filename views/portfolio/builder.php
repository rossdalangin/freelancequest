    ...
    <div class="bg-slate-900 border border-slate-800 p-6 sm:p-8 rounded-2xl space-y-6 shadow-xl">
        ...
        <form action="/portfolio-builder" method="POST" class="space-y-4">
            <div>
                ...
                <input type="text" name="title" value="<?= htmlspecialchars($portfolio['title'] ?? '') ?>" class="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-sm text-slate-100">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-300 mb-1">Tagline</label>
                <input type="text" name="tagline" value="<?= htmlspecialchars($portfolio['tagline'] ?? '') ?>" class="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-sm text-slate-100">
            </div>
            <div>
                <label class="block text-xs font-bold text-slate-300 mb-1">About Me / Value Proposition</label>
                <textarea name="about" rows="5" class="w-full bg-slate-950 border border-slate-800 rounded-lg p-2.5 text-sm text-slate-100"><?= htmlspecialchars($portfolio['about'] ?? '') ?></textarea>
            </div>
        ...
        <div class="pt-4 border-t border-slate-800 flex items-center justify-between text-xs">
            ...
            <a href="/p/<?= htmlspecialchars($portfolio['slug'] ?? 'user') ?>" target="_blank" class="text-indigo-400 font-bold hover:underline">freelancequest.com/p/<?= htmlspecialchars($portfolio['slug'] ?? 'user') ?> &rarr;</a>
        </div>
    </div>

    <div class="bg-slate-900 border border-slate-800 p-8 rounded-2xl shadow-2xl space-y-6">
        <h2 class="text-2xl font-black text-white"><?= htmlspecialchars($portfolio['title'] ?? '') ?></h2>
        <p class="text-indigo-400 font-semibold text-sm"><?= htmlspecialchars($portfolio['tagline'] ?? '') ?></p>
        <p class="text-xs text-slate-300 leading-relaxed"><?= htmlspecialchars($portfolio['about'] ?? '') ?></p>
    </div>
</div>
